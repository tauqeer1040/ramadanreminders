...
describe('schema version gate (subrequest budget regression)', () => {
  ...
  test('steady-state boot is cheap and preserves data', async () => {
    const uid = 'u-gate';
    await upsertUser(uid, 'T', `${uid}@t.example`);
    await upsertJournal(uid, { id: 'jg', text: 'gate journal text here' });
    const jrow = await db.execute({
      sql: `SELECT created_at FROM journal_entries WHERE id = ?`, args: ['jg'],
    });
    await decks.completeDeck(uid, 'jg', jrow.rows[0].created_at, CARDS('g'));
    ...
    db.execute = (...a) => { calls.push(a[0] && a[0].sql ? a[0].sql : a[0]); return orig.apply(db, a); };
    try {
      await initDB();
    } finally {
      db.execute = orig;
    }
    // probe + seed batch + rebuild count-check + processing reaper: must stay
    // far below the Workers ~50 subrequest budget (this gate is what keeps
    // cold boots alive; see the 1101 outage).
    expect(calls.length).toBeLessThanOrEqual(12);

    // Stale marker re-runs DDL idempotently without touching user data.
    await db.execute({
      sql: `UPDATE app_config SET value = '2' WHERE key = 'schema_version'`,
    });
    await initDB();
    ...
    const jstill = await db.execute({
      sql: `SELECT COUNT(*) AS n FROM journal_entries WHERE id = 'jg'`,
    });
    expect(Number(jstill.rows[0].n)).toBe(1);
  });
});
